...
{
    private static XMatShaper8Data malloc_aligned_sse(Context? ContextID)
    {
        var real_ptr = (byte*)NativeMemory.AllocZeroed((nuint)sizeof(XMatShaper8Data.Inner) + 32);
        var aligned = (byte*)(((ulong)real_ptr + 16) & ~(ulong)0xf);
        var p = (XMatShaper8Data.Inner*)aligned;

        p->real_ptr = real_ptr;

        return new XMatShaper8Data() {
            ContextID = ContextID,
            Data = p,
        };
    }

    private static void FillFirstShaper(Span<float>Table, ToneCurve Curve)
    {
        for (var i = 0; i < 256; i++)
        {
            var R = (float)(i / 255.0);
    ...
    private static void FillSecondShaper(Span<float> Table, ToneCurve Curve)
    {
    ...
    private static XMatShaper8Data? SetMatShaperSse(Context? ContextID,
                                                    ...
                                                    ReadOnlySpan<ToneCurve> Curve2)
    {
        ...
        var p = malloc_aligned_sse(ContextID);
        if (p is null) return null;

        p.ContextID = ContextID;

        // Precompute tables
        FillFirstShaper(new(p.Data->Shaper1R, 256), Curve1[0]);
        FillFirstShaper(new(p.Data->Shaper1G, 256), Curve1[1]);
        FillFirstShaper(new(p.Data->Shaper1B, 256), Curve1[2]);

        FillSecondShaper(new(p.Data->Shaper2R, 0x4001), Curve2[0]);
        FillSecondShaper(new(p.Data->Shaper2G, 0x4001), Curve2[1]);
        FillSecondShaper(new(p.Data->Shaper2B, 0x4001), Curve2[2]);
                                                 ...
                                                 ref uint dwFlags)
    {
...

        {
            ...
            else
            {
                ...
                UserData = SetMatShaperSse(ContextID, mpeC1.TheCurves, res, new VEC3(Data2.Offset), mpeC2.TheCurves);
