...
public static class WhitePoint
{
    ...
    {
        /** Original Code (cmswtpnt.c line: 47)
         **
        ...
        double x;
        if (t is >= 4000.0 and <= 7000.0)
        {
            x = (-4.6070 * (1e9 / t3)) + (2.9678 * (1e6 / t2)) + (0.09911 * (1e3 / t)) + 0.244063;
        }
        else if (t is > 7000.0 and <= 25000.0)
        {
            // or for correlated color temperature (T) between 7000K and 25000K:
            x = (-2.0064 * (1e9 / t3)) + (1.9018 * (1e6 / t2)) + (0.24748 * (1e3 / t)) + 0.237040;
        }
        ...
        {
            Errors.InvalidWhitePointFromTemp(null);
            return xyY.NaN;
        }

        // Obtain y(x)
        var y = (-3.0 * (x * x)) + (2.870 * x) - 0.275;

        // Wave factors (not used, but here for future extensions)

        //var m1 = (-1.3515 - (1.7703 * x) + (5.91140 * y)) / (0.0241 + (0.2562 * x) - (0.7341 * y));
        //var m2 = (0.0300 - (31.4424 * x) + (30.0717 * y)) / (0.0241 + (0.2562 * x) - (0.7341 * y));

        return (x, y, 1.0);
    }

    public static double ToTemp(xyY wp)
    {
        /** Original Code (cmswtpnt.c line: 142)
         **
         ** // Robertson's method
    ...
    internal static Mat3 AdaptationMatrix(Mat3? coneMatrix, XYZ fromIll, XYZ toIll)
    {
        ...
        var lamRigg = new Mat3(
            new(00.8951, 00.2664, -0.1614),
            new(-0.7502, 01.7135, 00.0367),
            new(00.0389, -0.0685, 01.0296)
        );

        coneMatrix ??= lamRigg;

        return ComputeChromaticAdaptation(fromIll, toIll, coneMatrix.Value);
    }

    internal static bool AdaptMatrixToD50(ref Mat3 r, xyY srcWp)
    {
        /** Original Code (cmswtpnt.c line: 247)
         **
        ...
        var bradford = AdaptationMatrix(null, dn, D50XYZ);
        if (bradford.IsNaN) return false;
    ...
    {
        /** Original Code (cmswtpnt.c line: 265)
         **
        ...
        // Build primaries matrix
        var p1 = new Vec3(xr, xg, xb);
        var p2 = new Vec3(yr, yg, yb);
        var p3 = new Vec3(1 - xr - yr, 1 - xg - yg, 1 - xb - yb);

        var primaries = new Mat3(p1, p2, p3);

        // Result = primaries ^ (-1) inverse matrix
        var result = primaries.Inverse();
        if (result.IsNaN) return Mat3.NaN;
        ...
        // Give us the Coefs, then I build transformation matrix
        p1 = new(coef.X * xr, coef.Y * xg, coef.Z * xb);
        p2 = new(coef.X * yr, coef.Y * yg, coef.Z * yb);
        p3 = new(coef.X * (1.0 - xr - yr), coef.Y * (1.0 - xg - yg), coef.Z * (1.0 - xb - yb));

        var r = new Mat3(p1, p2, p3);
        if (!AdaptMatrixToD50(ref r, whitePt)) return Mat3.NaN;
