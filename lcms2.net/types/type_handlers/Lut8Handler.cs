...
public class Lut8Handler : ITagTypeHandler
{
    ...
    public object? Read(Stream io, int sizeOfTag, out int numItems)
    {
        ...
        if (clutPoints == 1) goto Error; // Imposible value, 0 for not CLUT and at least 2 for anything else
        ...
        var numTabSize = IOHandler.Uipow(outputChannels, clutPoints, inputChannels);
        ...
        if (numTabSize > 0) {
            ...
            for (var i = 0; i < numTabSize; i++) {
                ptrW[0] = IOHandler.From8to16(temp[i]);
                ptrW = ptrW[1..];
            }
    ...
    public bool Write(Stream io, object value, int numItems)
    {
        ...
        var numTabSize = IOHandler.Uipow(newLut.OutputChannels, clutPoints, newLut.InputChannels);
        if (numTabSize == unchecked((uint)-1)) return false;
        if (numTabSize > 0) {
            // The 3D CLUT.
            if (clut is not null)
                for (var j = 0; j < numTabSize; j++) {
                    var val = IOHandler.From16to8(clut.Table.T[j]);
